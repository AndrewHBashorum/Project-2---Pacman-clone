...
// Load the font file & set up message
{
	...
	menuMessage.setFont(m_font);  // set the font for the text
	menuMessage.setCharacterSize(28); // set the text size
	menuMessage.setFillColor(sf::Color::White); // set the text colour
	menuMessage.setPosition(50, 150);  // its position on the screen
...
void Game::setUpMaze() // sets up 2d maze. sets cell type and displaces each cell into position
{
	...
	maze[0][5].typeOFCell = 0;
	maze[0][6].typeOFCell = 0;
	...
	maze[11][5].typeOFCell = 0;
	maze[11][6].typeOFCell = 0;
	...
	{
		float displacemantCol = row * 32; // 32 pixils in each image / finds x y pos of each cell

		for (int col = 0; col < MAX_COLS; col++)
		{
			float displacemantRow = col * 32; // finds x y pos of each cell
...
void Game::gameModeInput()
{
	...
	if (gameMode == GAME_PLAY)
	{
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::P))   // when in game over screen screen press r to go to GAME_PLAY screen
		{
			gameMode = MENU;
		}
	}
	...
	if (gameMode == MENU)
	{
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Escape))   // when in game over screen screen press r to go to GAME_PLAY screen
		{
			gameMode = GAME_PLAY;
...
// This function draws the game world
{
	...
	if (gameMode == MENU)
	{
		menuMessage.setString("-Press Escape to continue \n-Press R to Restart \n-Press Space to Enter new Name ");
		for (int row = 0; row < MAX_ROWS; row++)
		{
			for (int col = 0; col < MAX_COLS; col++)
			{
...
{
	int random = rand() % 4 + 1; // SETS RANDOM NUMBER so movemeant is random
...
void Game::pacmanAndText() // when pacman moves to the top left corner where the text is the text becomes transparent
{
	if (player.getRow() < 4 && player.getCol() < 4) // if in top left cornor of the screen
	{
		m_message.setFillColor(sf::Color(255, 255, 255, 175)); // transparent
		healthMessage.setFillColor(sf::Color(255, 255, 255, 175));
		nameDisplay.setFillColor(sf::Color(255, 255, 255, 175));
	}
	else // if not top left corner
	{
		m_message.setFillColor(sf::Color(255, 255, 255, 255)); // full white
		healthMessage.setFillColor(sf::Color(255, 255, 255, 255));
		nameDisplay.setFillColor(sf::Color(255, 255, 255, 255));
	}
}

void Game::userInput(sf::Event t_event) // counter is is used to animate ghost
{
